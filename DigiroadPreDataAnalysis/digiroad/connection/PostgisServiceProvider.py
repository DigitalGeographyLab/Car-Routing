...
from digiroad.util import getConfigurationProperties, GPD_CRS, getFormattedDatetime, timeDifference
...
class PostgisServiceProvider(AbstractGeojsonProvider):
    ...
    def executePostgisQuery(self, sql):
        ...
        newJson = self.convertToGeojson(df)
    ...
    def getNearestCarRoutableVertexFromAPoint(self, coordinates, radius=500):
        """
        From the Database retrieve the nearest car routing vertex from a given point coordinates.

        :param coordinates: Point coordinates. e.g [889213124.3123, 231234.2341]
        :return: Geojson (Geometry type: Point) with the nearest point coordinates.
        """

        print("Start getNearestCarRoutableVertexFromAPoint")

        epsgCode = coordinates.getEPSGCode().split(":")[1]

        sql = self.getNearestCarRoutableVertexSQL(coordinates, epsgCode, radius)
        ...
        print("End getNearestCarRoutableVertexFromAPoint")
        return geojson

    def getNearestCarRoutableVertexSQL(self, coordinates, epsgCode, radius):
        return "SELECT " \
               "v.id," \
               "ST_SnapToGrid(v.the_geom, 0.00000001) AS geom, " \
               "string_agg(distinct(e.old_id || ''),',') AS name " \
    ...
    def getShortestPath(self, startVertexId, endVertexId, cost):
        ...
        sql = "SELECT " \
              ...
              "FROM edges_noded', %s, %s, true, true) AS r, " \
              "edges_noded AS e " \
              "WHERE " \
              "r.id2 = e.id " \
    ...
    def getTotalShortestPathCostManyToOne(self, startVerticesID=[], endVertexID=None, costAttribute=None):
        ...
        sql = "SELECT " \
              ...
              "ELSE -1 " \
              "END)::double precision AS reverse_cost " \
              "FROM edges_noded\', ARRAY[%s], %s, true)) as r," \
              "edges_noded_vertices_pgr AS s," \
              "edges_noded_vertices_pgr AS e " \
              "WHERE " \
              "s.id = r.start_vid " \
    ...
    def getTotalShortestPathCostOneToMany(self, startVertexID=None, endVerticesID=[], costAttribute=None):
        ...
        sql = "SELECT " \
              ...
              "ELSE -1 " \
              "END)::double precision AS reverse_cost " \
              "FROM edges_noded\', %s, ARRAY[%s], true)) as r," \
              "edges_noded_vertices_pgr AS s," \
              "edges_noded_vertices_pgr AS e " \
              "WHERE " \
              "s.id = r.start_vid " \
    ...
    #           "ELSE -1 " \
    #           "END)::double precision AS reverse_cost " \
    #           "FROM edges_noded\', ARRAY[%s], ARRAY[%s], true)) as r," \
    #           "edges_noded_vertices_pgr AS s," \
    #           "edges_noded_vertices_pgr AS e " \
    #           "WHERE " \
    #           "s.id = r.start_vid " \
    ...
    def getTotalShortestPathCostManyToMany(self, startVerticesID=[], endVerticesID=[], costAttribute=None):
        ...
        geojson = self.convertToGeojson(dataFrame)
